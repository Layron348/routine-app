...
export const InsightsView: React.FC<Props> = ({ stats, plan, onAdd }) => {
  const [activeTab, setActiveTab] = useState<"overview" | "calendar" | "achievements">("overview");
  const today = new Date().toISOString().split("T")[0];
  const allTasks = plan.days.flatMap(d => d.tasks);
  const mockRewards = {
    xp: stats.done * 10,
    coins: stats.streak * 5,
    level: Math.floor(stats.streak / 7) + 1,
  };
  ...
  return (
    <div className="insights-view">
            ...
            {stats.by_day.map(d => (
              <div key={d.date} className="stats-day-row">
                ...
                <div className="sdr-bar">
                  <div className={`sdr-fill${d.date === today ? " today" : ""}`} style={{ width: `${d.percent}%` }} />
                </div>
      ...
      {activeTab === "calendar" && (
        <CalendarView plan={plan} onDateSelect={(date) => onAdd(date, "New task", "routine", "medium", null, null, false)} />
      )}

      {activeTab === "achievements" && (
        <>
          <RewardSystem rewards={mockRewards} />
